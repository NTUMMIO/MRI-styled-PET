...
root_folders = ["/home/linyunong/project/pvc/src/output/AD", "/home/linyunong/project/pvc/src/output/MCI", "/home/linyunong/project/pvc/src/output/CN"]
# file_names = ["PET.nii", "output_RBV_suvr.nii", "fusion.nii"]
fusion_root_folders=['/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/output_nifti/fusion/ConfigB_ratio05_anatomical3/fold0/2024-9-18-13836_train3_inference7/AD', '/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/output_nifti/fusion/ConfigB_ratio05_anatomical3/fold0/2024-9-18-13836_train3_inference7/MCI', '/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/output_nifti/fusion/ConfigB_ratio05_anatomical3/fold0/2024-9-18-13836_train3_inference7/CN']

file_names = ["PET.nii", "output_IY_suvr.nii", "fusion_best.nii"]
source_names = ["PET", "PVC", "MRI-styled PET", "FS_mask", "FS_aseg_mni"]
...
def create_box_plot(data_all, group_name, edge_color, fill_color, x_title):
    ...
    plt.savefig('/home/linyunong/project/style_transfer/analysis/new_sessions/box_plot/{}.png'.format(x_title))

def create_bland_altman_plot(data_all, subject_names, region, cohens_d_data):
    group_names=['CN', 'MCI', 'AD']
    modality_names=["PET", "PVC", "MRI-styled PET"]
    color_index=[
    ...
    for i in range(len(group_names)):
        ...
        print(title_name, data_all.shape)  
        ...
        row=[region, group_names[i], cohens_d_1, cohens_d_2]
        ...
        plt.savefig('/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/analysis/results/old_bland_altman/'+title_name+'.png')
        plt.close()

        # os.mkdir("/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/analysis/results/old_bland_altman/{}".format(title_name))

        # print(np.array(subject_names[2-i][:, 0]).shape, data_all[i][0].shape)

        if title_name.find('CN')==-1:
            pet=pd.DataFrame(np.swapaxes(np.array([subject_names[2-i], data_all[i][0]]), 0, 1))#, columns=['Subject', "SUVr"])
            pet.to_csv("/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/analysis/results/old_bland_altman/{}/PET.csv".format(title_name))
            pvc=pd.DataFrame(np.swapaxes(np.array([subject_names[2-i], data_all[i][1]]), 0, 1))#, columns=['Subject', "SUVr"])
            pvc.to_csv("/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/analysis/results/old_bland_altman/{}/PVC.csv".format(title_name))
            mr_styled_pet=pd.DataFrame(np.swapaxes(np.array([subject_names[2-i], data_all[i][2]]), 0, 1))#, columns=['Subject', "SUVr"])
            mr_styled_pet.to_csv("/home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/analysis/results/old_bland_altman/{}/MR-styled-PET.csv".format(title_name))
    ...
    image_paths = ['/home/linyunong/project/style_transfer/analysis/new_sessions/bland_altman/'+region+'(CN)', 
                   '/home/linyunong/project/style_transfer/analysis/new_sessions/bland_altman/'+region+'(MCI)', '/home/linyunong/project/style_transfer/analysis/new_sessions/bland_altman/'+region+'(AD)']
    # return cohens_d_data
    # # Load images
    # images = []
    ...
    # image_shape = images[0].shape[:2]  # Get height and width
    # for image in images[1:]:
    #     if image.shape[:2] != image_shape:
    #         raise ValueError("Images must have the same shape!")

    # # Calculate total width (assuming RGB channels)
    # total_width = len(images) * image_shape[1]

    # # Create a black canvas with the combined width and original height
    # combined_image = np.zeros((image_shape[0], total_width, 3), dtype=np.uint8)

    # # Place each image on the canvas horizontally
    # start_x = 0
    # for image in images:
    #     combined_image[:, start_x:start_x + image_shape[1], :] = image
    #     start_x += image_shape[1]
    ...
    # print("Images successfully loaded and combined!")
    
    # img_path='/home/linyunong/project/style_transfer/analysis/new_sessions/bland_altman/'
 
    # new_im = Image.new('RGB', (600*len(group_names),500))
    # for i in range(len(group_names)):
    #     path=img_path+region+'('+group_names[i]+').png'
    #     print(path)
    #     im=Image.open(path)
    #     im=im.resize((600,500))
    #     new_im.paste(im, (600*i,500))
                

    # new_im.save('/home/linyunong/project/style_transfer/analysis/new_sessions/bland_altman/'+region+'(all).png')
...
if __name__ == "__main__":
    ...
    cohens=[['Region', 'Group', 'PVC vs PET', 'MRI-styled PET vs PET']]
    
    subject_names=[]
    # Iterate through root folders
    for i in range(len(root_folders)):
        root_folder=root_folders[i]
        subject_names_each_group=[]
        print(root_folder, len(os.listdir(root_folder)))
        ...
        for subject_folder in os.listdir(root_folder):
            ...
            subject_names_each_group.append(subject_folder)
            # =================================================
            # 1. Record PET, PVC, MRI-styled PETof each subject
            ...
            for f in range(len(file_names)):
                if file_names[f].find("016_S_4952")==-1:
                    ...
                    if file_name=='fusion_best.nii':
                        file_path = os.path.join(fusion_root_folders[i], subject_folder+'.nii')
                        print(file_path)
                    else:
        ...
        subject_names.append(subject_names_each_group)
    ...
    for region_num, region_dict in region_json.items():
        ...
        if region_num!="GM-WM ratio" and region_num!="GM SUVr"and region_num!="WM SUVr":
            ...
            for nii in data_arrays.keys():
                if nii.find("FS_mask")==-1 and nii.find("FS_aseg_mni")==-1:
                    ...
                    print(nii, groups_suvr_array.shape)
        ...
        # subject_names=np.array(subject_names[0])       
        # print(np.array(subject_names[0]).shape, np.array(subject_names[1]).shape, np.array(subject_names[2]).shape)

        # subject_names=np.array([subject_names[0],  subject_names[50:100], subject_names[:50]])
        # # subject_names=np.swapaxes(np.array(subject_names), 0, 1)
        ...
        cohens=create_bland_altman_plot(data, subject_names, region, cohens)
    ...
    df.to_csv("//home/linyunong/project/MRI-styled-PET-Dual-modality-Fusion-for-PET-Image-Enhancement/analysis/results/cohensd/cohens_d.csv")
